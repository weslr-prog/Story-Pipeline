...
class Settings:
    ...
    word_target_min: int = _env_int("WORD_TARGET_MIN", 2000)
    word_target_max: int = _env_int("WORD_TARGET_MAX", 2600)
    ...
    chapter1_forbidden_terms: tuple[str, ...] = _env_csv(
        ...
        "for elara,hidden door,novaBio tracker,tracker lay hidden",
    )
    ...
    silence_pad: float = _env_float("SILENCE_PAD", 0.45)
    sample_rate: int = _env_int("SAMPLE_RATE", 22050)
    request_delay: float = _env_float("REQUEST_DELAY", 0.60)
    max_retries: int = _env_int("MAX_RETRIES", 3)
    retry_backoff: float = _env_float("RETRY_BACKOFF", 0.80)
